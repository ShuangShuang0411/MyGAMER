...
//     --> a helper macro PRINT_WARNING is defined in TestProb.h
   const double End_T_Default    = 2.0e-2;
...
void BC_User_Wind( real fluid[], const double x, const double y, const double z, const double Time,
                       const int lv, double AuxArray[] )
{

   const double Vx = 1.25;

   double Dens, MomX, MomY, MomZ, Pres, Eint, Etot;

   if (x <= 0.0)
      Dens = 1.0e-3*Blast_Dens_Bg;
      MomX = Dens*Vx;
      MomY = 0.0;
      MomZ = 0.0;
      Pres = Blast_Pres_Bg;

...
//     --> a helper macro PRINT_WARNING is defined in TestProb.h
   ...
   const double End_T_Default    = 2*BOX_SIZE * UNIT_L / (CharacteristicSpeed *UNIT_V) / UNIT_T;
...
// variables for jet
   ...
   if ( Jet_Ambient == 0 ) // uniform ambient
   {
      ...
      //set the neutron star density
      double R = 5*Jet_Radius;
      if (xp[0]*xp[0]+xp[1]*xp[1]+xp[2]*xp[2] <= R*R){
         Pri[0] = (real)Amb_UniformDens*10000;
      }
      else{
